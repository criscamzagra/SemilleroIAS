...
newProducts.forEach((newProduct) => {
  ...
  const typeOfProductsExist = typeOfProducts.find((typeOfProduct) => { return typeOfProduct.name === newProduct.typeOfProcuct });

  if (typeOfProductsExist != undefined) {
    typesValidProducts.push({ id: newProduct.id, status: 'succes' });

  } else {
    typesValidProducts.push({ id: newProduct.id, status: 'error', message: 'there is no product type' });
  }

});


const newListProductsValidation = [...typesValidProducts]

console.log(newListProductsValidation);
products.forEach((product) => {

  const validProductId = typesValidProducts.find((typesValidProduct) => { return typesValidProduct.id === product.productId });

  if (validProductId != undefined) {
    newListProductsValidation.push({ id: product.productId, status: 'error', message: 'There is already a product with the id' })

  } else {
    newListProductsValidation.push({ id: product.productId, status: 'succes' });
  }
